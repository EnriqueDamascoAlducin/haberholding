<?php  
	...
	$usuarios = $con->query("Select id_usu as id, CONCAT(IFNULL(nombre_usu,''),' ',IFNULL(apellidop_usu,''), ' ',IFNULL(apellidom_usu,'')) as nombre, nombre_depto, nombre_puesto,ingreso_usu,noempleado_usu as noempleado FROM clientes u Inner join departamentos on u.depto_usu = id_depto INNER JOIN puestos on u.puesto_usu = id_puesto WHERE status_usu<>0");
...
<script type="text/javascript">
	function agregarUsuario(){
		...
		$("#modalContenido").load("vistas/clientes/formularios/form1.php");
	}
	function editarUsuario(id){
		$("#modaltitulo").html("Editar Usuario");
		$("#modalContenido").load("vistas/clientes/formularios/form1.php",{id:id});
	}
