				...
				<tr>
					...
					<td>
						<?php if(in_array("EDITAR", $permisosActuales)){ ?>
							<i class="fas fa-edit"  data-toggle="modal" data-target="#modal" onclick="editar(<?php echo $marca['id']; ?>)"></i>
						<?php } ?>
						<?php if(in_array("ELIMINAR", $permisosActuales)){ ?>
							<i class="fa fa-trash" onclick="eliminar(<?php echo $marca['id']; ?>,'<?php echo $marca["nombre"] ?>')"></i>
						<?php } ?>
						<?php if(in_array("MODELOS", $permisosActuales)){ ?>
							<i class="fa fa-certificate" data-toggle="modal" data-target="#modal" onclick="modelos(<?php echo $marca['id']; ?>,'<?php echo $marca["nombre"] ?>')"></i>
						<?php } ?>
					</td>
				</tr>
			<?php } ?>
...
<script type="text/javascript">
	function agregar(){
		...
		$("#modaltitulo").html("Registrar Marca");
	...
	function modelos(id,nombre){

		$("#enviarForm").html("Enviar").removeClass('btn-success').addClass('btn-primary');

		$("#modaltitulo").html("Registrar Modelo de " + nombre );
		$("#modalContenido").load("vistas/<?php echo $rutaModulo; ?>/formularios/modelos.php",{id:id,nombre:nombre});
	}
