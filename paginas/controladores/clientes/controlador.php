...
include_once '../finsession.php';
	if(isset($_POST['opc']) && $_POST['opc'] == "agregar"){
		...
		if(sizeof($errores)==0){
			...
			$query = "INSERT INTO clientes (nombre_usu,apellidop_usu,apellidom_usu,sexo_usu,correo_usu,noempleado_usu,depto_usu,puesto_usu,extension_usu,contrasena_usu,ingreso_usu) VALUES (".$valores.")";
		...
		if(isset($_POST['contrasena']) && !empty(trim($_POST['contrasena']))){
			...
			if(sizeof($errores)==0){
				...
				$ingreso = "'".trim($_POST['ingreso'])."'";
				$contrasena = "'".md5($_POST['contrasena'])."'";
				$query = "UPDATE clientes SET noempleado_usu = ". $noEmpleado.", nombre_usu=".$nombre.",apellidop_usu=".$apellidop.",apellidom_usu=".$apellidom.",sexo_usu=".$sexo.",correo_usu=".$correo.",depto_usu=".$departamento.",puesto_usu=".$puesto.",extension_usu=".$extension.",contrasena_usu=".$contrasena.",ingreso_usu=".$ingreso." WHERE id_usu = ".$idusu;
			}
		}else{
			if(sizeof($errores)==0){
				if(empty(trim($_POST['apellidom']))){
				...
				$puesto = trim($_POST['puesto']);
				$ingreso = "'".trim($_POST['ingreso'])."'";
				$query = "UPDATE clientes SET noempleado_usu = ". $noEmpleado.", nombre_usu=".$nombre.",apellidop_usu=".$apellidop.",apellidom_usu=".$apellidom.",sexo_usu=".$sexo.",correo_usu=".$correo.",depto_usu=".$departamento.",puesto_usu=".$puesto.",extension_usu=".$extension.",ingreso_usu=".$ingreso." WHERE id_usu = ".$idusu;
			}
		...
		$id = $_POST['usuario'];
		$query = "UPDATE clientes set status_usu = 0 where id_usu =".$id;
